...
-module(nkserver_rpc9_server).
-author('Carlos Gonzalez <carlosj.gf@gmail.com>').

-export([start_link/2, get_sup_spec/2]).
-export([stop/1, update/2]).
-export([send_request/3, send_async_request/3, send_event/3, reply/2]).
...
update(Id, Config) ->
    ...
    Config3 = case Config2 of
        #{plugins:=Plugins} ->
            Config2#{plugins:=[nkserver_rpc9_server|Plugins]};
        _ ->
            Config2
    end,
...
send_request(SrvId, Cmd, Data) ->
    case get_pid(SrvId) of
        Pid when is_pid(Pid) ->
            nkserver_rpc9_server_protocol:send_request(Pid, Cmd, Data);
        undefined ->
            {error, no_transports}
    end.
...
send_async_request(SrvId, Cmd, Data) ->
    case get_pid(SrvId) of
        Pid when is_pid(Pid) ->
            nkserver_rpc9_server_protocol:send_async_request(Pid, Cmd, Data);
        undefined ->
            {error, no_transports}
    end.
...
send_event(SrvId, Event, Data) ->
    case get_pid(SrvId) of
        Pid when is_pid(Pid) ->
            nkserver_rpc9_server_protocol:send_event(Pid, Event, Data);
        undefined ->
            {error, no_transports}
    end.
...
reply(#{session_pid:=Pid, tid:=TId}, Reply) ->
    nkserver_rpc9_server_protocol:reply(Pid, TId, Reply).


%% @private
get_pid(SrvId) ->
    case nkserver_rpc9_server_protocol:get_local_started(SrvId) of
        [Pid|_] ->
            Pid;
        [] ->
            undefined
